...
class FragmentNewsFeed : Fragment(),RVOnItemClickListener, RVOnSubItemClickListener {
    ...
    var gettedSharedPostCounter=0
    var sharedPostCount=0
...
/*        Handler().postDelayed({
    ...
    override fun onItemClicked(view: View, position: Int) {
        if(view.id== R.id.linearDots){
            ...
            MyApplication.selectedPostUserId = MyApplication.selectedPost.userId
            var x =  MyApplication.selectedPost.userId
...
/*            arrayFeeds[position].settingsViewVisible=!arrayFeeds[position].settingsViewVisible!!
      ...
      /*      arrayFeeds[position].settingsViewVisible=!arrayFeeds[position].settingsViewVisible!!
            ...
            selectShare(activity!!,position,arrayFeeds[position].id!!)
    ...
    private fun onPostRetreived(response: Response<ArrayList<ResponsePost>>){
        ...
        try{
            ...
            gettedSharedPostCounter=0
            sharedPostCount=0
            for (i in response.body()!!.indices) {
                if (response.body()!![i].Shared!!)
                    sharedPostCount++
            }
            Log.wtf("shared_total_count",sharedPostCount.toString())
            if(sharedPostCount==0)
                loading.visibility=View.GONE

            for (i in response.body()!!.indices){
                if(response.body()!![i].Shared!!)
                  getPostShare(response.body()!![i].id!!,response.body()!![i].SharedPostId!!)
            }
    ...
    fun getPostShare(postId:Int,sharedPostId: Int){
        ...
        RetrofitClientAuth.client?.create(RetrofitInterface::class.java)
            ...
            )?.enqueue(object : Callback<ResponseSharePost> {
                override fun onResponse(call: Call<ResponseSharePost>, response: Response<ResponseSharePost>) {
                    try{
                        gettedSharedPostCounter++
                        addSharedPost(postId,response.body()!!)
                        Log.wtf("shared_current_count",gettedSharedPostCounter.toString())
                        if(gettedSharedPostCounter==sharedPostCount) {
                            loading.visibility = View.GONE
                            try{adapternewsfeed!!.notifyDataSetChanged()}catch (e:Exception){}
                        }

                    }catch (E:Exception){
                        Log.wtf("get_post_error", E.toString())
                        Log.wtf("shared_current_count",E.toString())
                    }
                }
                override fun onFailure(call: Call<ResponseSharePost>, throwable: Throwable) {
                    try{loading.visibility=View.GONE}catch (e:java.lang.Exception){}
                    Toast.makeText(activity,"failed",Toast.LENGTH_LONG).show()
                }
            })
    ...
    private fun addSharedPost(id: Int,response: ResponseSharePost){
        val feed: ResponsePost? = arrayFeeds.find { it.id == id }
        arrayFeeds[arrayFeeds.indexOf(feed)].sharedPost=response
    }
    ...
    private fun initFilters(){
        ...
        for (i in MyApplication.arrayCategories.indices){
            if(MyApplication.languageCode==AppConstants.LANG_ENGLISH)
                ...
                arraySpinnerCategories.add(ItemSpinner(MyApplication.arrayCategories[i].id,MyApplication.arrayCategories[i].valueAr,MyApplication.arrayCategories[i].iconPathAr))
       }
...
//        val radio:RadioButton = activity!!.findViewById(rgProblems!!.checkedRadioButtonId)
    ...
    private fun selectShare(context: Context,position: Int,postId: Int) {
        var options= arrayOf<CharSequence>()

        if(arrayFeeds[position].userId!=MyApplication.userLoginInfo.id) {
        ...
        builder.setItems(options) { dialog, item ->
            when {
                options[item] == getString(R.string.share) -> share(position)
                options[item] == getString(R.string.share_page) -> sharePage(postId)
    ...
    private fun sharePage(postId: Int){
        loading.visibility = View.VISIBLE
            RetrofitClientAuth.client?.create(RetrofitInterface::class.java)
                ?.shareTimeline(
                    postId
