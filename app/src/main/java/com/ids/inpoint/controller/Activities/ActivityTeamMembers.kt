...
import kotlinx.android.synthetic.main.activity_team_members.btClose
...
class ActivityTeamMembers : AppCompactBase(), RVOnItemClickListener {

    private var lastFragmentId = 0
    private lateinit var adapterGridTeamMember: AdapterGridTeamMember
    private var arrayFilteredMembers = java.util.ArrayList<TeamStartupMember>()
    private var arrayMembers = java.util.ArrayList<TeamStartupMember>()
    ...
    private fun init() {
        ...
        etSearchMember!!.addTextChangedListener(object: TextWatcher {override fun afterTextChanged(s: Editable?) {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                filterMembers(s.toString().toLowerCase())
            }
        ...
        btClose.setOnClickListener{
            ...
            try{adapterGridTeamMember.notifyDataSetChanged()}catch (e:java.lang.Exception){}
            try{hideKeyboard(it) }catch (e: java.lang.Exception){}
        }
    }


    private fun filterMembers(word:String){
        if(arrayMembers.size>0 && word.isNotEmpty()){
            btClose.visibility=View.VISIBLE
            arrayFilteredMembers.clear()
            for(i in arrayMembers.indices){
                if(arrayMembers[i].name!!.toLowerCase().contains(word) || arrayMembers[i].name!!.toLowerCase().contains(word))
                    arrayFilteredMembers.add(arrayMembers[i])
            }
        }else {
            arrayFilteredMembers.clear()
            btClose.visibility=View.GONE
            arrayFilteredMembers.addAll(arrayMembers)
        }
        try{ adapterGridTeamMember.notifyDataSetChanged()}catch (e: java.lang.Exception){}
    }
        ...
        RetrofitClientAuth.client?.create(RetrofitInterface::class.java)
            ?.getTeamMembers(id)?.enqueue(object : Callback<ArrayList<TeamStartupMember>> {
                ...
                override fun onFailure(
                    ...
                    loading.visibility = View.GONE;
                    Toast.makeText(this@ActivityTeamMembers, getString(R.string.failed), Toast.LENGTH_LONG).show()
                }
            })
    }
...
//        }
    ...
    private fun saveTeamMember() {
        loading.visibility = View.VISIBLE;
        RetrofitClientAuth.client?.create(RetrofitInterface::class.java)
            ?.saveTeamMember(teamMember)?.enqueue(object : Callback<Void> {
    ...
    private fun deleteTeamMember() {
        loading.visibility = View.VISIBLE;
        RetrofitClientAuth.client?.create(RetrofitInterface::class.java)
            ?.deleteTeamMember(teamMember.id!!)?.enqueue(object : Callback<Void> {
                override fun onResponse(call: Call<Void>, response: Response<Void>) {
                    getTeamMembers(MyApplication.selectedStartupTeam.id!!)
                }
    ...
    private fun filterUsers(word: String) {
        if (MyApplication.arrayUsers.size > 0) {
            ...
            var notMembers =
                MyApplication.arrayUsers.filter { ru -> arrayFilteredMembers.all { it.userId != ru.id } }
    ...
    override fun onItemClicked(view: View, position: Int) {
        when (view.id) {
            R.id.linearItemUser -> {
                ...
                teamMember.teamId = MyApplication.selectedStartupTeam.id
            ...
            R.id.btDelete -> {
                teamMember = arrayFilteredMembers[position]
            ...
            R.id.btEdit -> {
                ...
                etUser.setText(arrayFilteredMembers[position].name)
                swAdmin.isChecked = arrayFilteredMembers[position].admin == true
                edit = false
                etUser.isEnabled = false
                btClear.isEnabled = false
            }
                ...
                if (MyApplication.userLoginInfo.id!! == arrayFilteredMembers[position].userId!!) {
