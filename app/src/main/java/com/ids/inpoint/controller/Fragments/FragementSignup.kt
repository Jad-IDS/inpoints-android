...
class FragementSignup : Fragment(), RVOnItemClickListener {
    ...
    private fun signup() {
        if (etName.text.toString().isEmpty() || etalender.text.toString().isEmpty() || etEmail.text.toString().isEmpty() || etPassword.text.toString().isEmpty())
            Toast.makeText(
                activity,
                getString(R.string.check_empty_fields),
                Toast.LENGTH_LONG
            ).show()
       ...
       else if (!AppHelper.checkPassword(etPassword.text.toString()).equals("correct"))
            createDialog(activity!!,AppHelper.checkPassword(etPassword.text.toString()))
        ...
        RetrofitClient.client?.create(RetrofitInterface::class.java)
            ?.registration(
                ...
                override fun onResponse(call: Call<String>, response: Response<String>) {
                    ...
                    try {
                        createDialog(activity!!, response.body().toString())
                    } catch (e: Exception) {
                    }
