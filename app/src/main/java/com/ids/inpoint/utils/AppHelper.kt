...
class AppHelper {
    ...
    companion object {
        ...
        fun checkPassword(password : String):String{
            ...
            var returnValue : String ="Problems with Password:\n"
            var changed:Boolean = false


            var letterLow : Pattern = Pattern.compile("[a-z]")
            var letterCapital : Pattern = Pattern.compile("[A-Z]")
            var digit : Pattern = Pattern.compile("[0-9]")
            var special : Pattern = Pattern.compile("[!@#$%&*()_+=|<>?{}\\[\\]~-]")

            var hasCapital : Matcher = letterCapital.matcher(password)
            var hasLower : Matcher = letterLow.matcher(password)
            var hasDigit : Matcher = digit.matcher(password)
            var hasSpecial:Matcher = special.matcher(password)

            if(password.length < 8){
                changed = true
                returnValue= returnValue + "Has less than 8 characters\n"
            }
            if(!hasCapital.find()) {
                changed = true
                returnValue = returnValue + "Needs Capital Letter\n"
            }
            if(!hasLower.find()) {
                changed = true
                returnValue = returnValue + "Needs Lowercase Letter\n"
            }
            if(!hasDigit.find()) {
                changed = true
                returnValue = returnValue + "Needs Digit\n"
            }
            if(!hasSpecial.find()) {
                changed = true
                returnValue = returnValue + "Needs Special Character\n"
            }
